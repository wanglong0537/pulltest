...
import com.kingsoft.email.widget.pullToRefresh.internal.FlipLoadingLayout;
import com.kingsoft.email.widget.pullToRefresh.internal.LoadingLayout;
import com.kingsoft.email.widget.pullToRefresh.internal.RotateLoadingLayout;
import com.kingsoft.email.widget.pullToRefresh.internal.Utils;
import com.kingsoft.email.widget.pullToRefresh.internal.ViewCompat;
...
//	public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 325;
	...
	public void setHeaderText (String head,String relase,String refreshing){
	    mHeaderLayout.setPullLabel(head);
	    mHeaderLayout.setReleaseLabel(relase);
	    mHeaderLayout.setRefreshingLabel(refreshing);
	    mHeaderLayout.reset();
	}
	public void setFooterText (String foot,String relase,String refreshing){
	    mFooterLayoutOther.setPullLabel(foot);
	    mFooterLayoutOther.setReleaseLabel(relase);
	    mFooterLayoutOther.setRefreshingLabel(refreshing);
	    mFooterLayoutOther.reset();
//	    final LinearLayout.LayoutParams lp = getLoadingLayoutLayoutParams();
//	    addViewInternal(mFooterLayout, lp);
	}
	public void hideImageHead(){
	    mHeaderLayout.hideImageHead();
	}
	public void showImageHead(){
	    mHeaderLayout.showImageHead();
	}
	
	public LoadingLayout changeFooter(){
	    return mFooterLayoutOther;
	}
	...
	public void setFootershowflag(boolean footershowflag){
	    this.footershowflag = footershowflag;
	}
	...
	protected void onPullToRefresh() {
		switch (mCurrentMode) {
			case PULL_FROM_END:
				...
				mFooterLayoutOther.pullToRefresh();
				break;
	...
	protected void onReleaseToRefresh() {
		switch (mCurrentMode) {
			case PULL_FROM_END:
				...
				mFooterLayoutOther.releaseToRefresh();
				break;
			case PULL_FROM_START:
	...
	protected final void refreshLoadingViewsSize() {
		...
		System.out.println(maximumPullScroll+"===================maximumPullScroll");
	...
	protected final void setHeaderScroll(int value) {
		...
		if (mLayoutVisibilityChangesEnabled) {
			if (value < 0) {
			    ...
			    if(footershowflag){
				mFooterLayout.setVisibility(View.VISIBLE);//modify by debby
			    }else{
				mFooterLayout.setVisibility(View.INVISIBLE);//modify by debby
			    }
	...
	private void addRefreshableView(Context context, T refreshableView) {
		mRefreshableViewWrapper = new ListViewFrame(context);
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mFooterLayoutOther = createLoadingLayout(context, Mode.PULL_FROM_END, a);
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			float caldimension = itemDimension;
			switch (mCurrentMode) {
				case PULL_FROM_END:
					mFooterLayout.onPull(scale);
					caldimension = itemDimension/2;
					break;
				case PULL_FROM_START:
				default:
			...
			if (mState != State.PULL_TO_REFRESH && caldimension >= Math.abs(newScrollValue)) {
				setState(State.PULL_TO_REFRESH);
			} else if (mState == State.PULL_TO_REFRESH && caldimension < Math.abs(newScrollValue)) {
				setState(State.RELEASE_TO_REFRESH);
			}
			if(mState == State.RESET && caldimension < Math.abs(newScrollValue)&&mFooterLayout.pulltorefreshflag==false){
			    setState(State.PULL_TO_REFRESH);
			}
		}
	}
